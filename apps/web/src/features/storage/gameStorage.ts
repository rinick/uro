import {getGameInfo, parseSgf, serializeSgf, type SgfDocument} from "@uro/sgf-core";
...
export async function listStoredGames(): Promise<StoredGameSummary[]> {
  const db = await openDatabase();
  const records = await requestToPromise<StoredGameRecord[]>(db.transaction(storeName, "readonly").objectStore(storeName).getAll());
  db.close();
  return records
    .map(({sgf: _sgf, ...summary}) => summary)
    .sort((left, right) => right.updatedAt - left.updatedAt);
}

export async function loadStoredGame(id: string): Promise<SgfDocument> {
  const db = await openDatabase();
  const record = await requestToPromise<StoredGameRecord | undefined>(db.transaction(storeName, "readonly").objectStore(storeName).get(id));
  db.close();
