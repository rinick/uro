  ...
  return (
    <div className="editor-toolbar">
      <Segmented
        ...
        options={[
          {value: "auto", label: withTip(<AutoPlayIcon nextColor={nextColor} onClick={onAutoToolClick} />, t("tools.auto"))},
      ...
      <Space.Compact className="navigation-tools">
        <NavButton title={t("nav.first")} disabled={!canNavigatePrevious} icon={<StepBackwardOutlined />} onClick={onFirst} />
        <NavButton title={t("nav.previous10")} disabled={!canNavigatePrevious} icon={<FastBackwardOutlined />} onClick={onPrevious10} />
        <NavButton title={t("nav.previous")} disabled={!canNavigatePrevious} icon={<BackwardOutlined />} onClick={onPrevious} />
        <NavButton title={t("nav.next")} disabled={!canNavigateNext} icon={<ForwardOutlined />} onClick={onNext} />
        <NavButton title={t("nav.next10")} disabled={!canNavigateNext} icon={<FastForwardOutlined />} onClick={onNext10} />
...
function NavButton({title, icon, disabled, onClick}: {title: string; icon: React.ReactNode; disabled: boolean; onClick: () => void}) {
  return (
    <Tooltip title={title}>
      <span>
